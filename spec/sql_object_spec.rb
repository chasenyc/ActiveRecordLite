...
describe SQLObject do
  ...
  describe '::columns' do
    ...
    it '::columns creates setter methods for each column' do
      c = Cat.new
      c.name = "Michael Jackson"
      c.id = 209
      c.owner_id = 2
      expect(c.name).to eq 'Michael Jackson'
    ...
    it '::columns created setter methods use attributes hash to store data' do
      ...
      expect(c.attributes[:name]).to eq 'Michael Jackson'
    end
  end

  describe '#initialize' do
    it '#initialize properly sets values' do
      c = Cat.new(name: 'Miles Davis', id: 100, owner_id: 4)
      expect(c.name).to eq 'Miles Davis'
    ...
    it '#initialize throws the error with unknown attr' do
      expect do
        Cat.new(favorite_band: 'Jackson Five')
  ...
  describe '#update' do
    it '#update changes attributes' do
      ...
      human.fname = 'Codester'
      human.lname = 'von Slacker'
      human.update

      human = Human.find(2)
      expect(human.fname).to eq('Codester')
      expect(human.lname).to eq('von Slacker')
    end
  end
